...
public class MinFourHeapTests {
    ...
    public void stressTests() {
        PriorityWorkList<Integer> heap = new MinFourHeap<>(Integer::compareTo);
        addAll(heap, new Integer[] {50, 49, 48, 47, 46, 45, 44, 43, 42, 41, 40, 39, 38, 37});

        Object[] heapData = getField(heap, "data");
        String heapStr = Arrays.toString(heapData);
        String heapExp = "[37, 43, 39, 38, 47, 50, 46, 45, 44, 49, 42, 41, 40, 48,";
        assertTrue(heapStr.contains(heapExp));


        heap.next();
        heapData = getField(heap, "data");
        heapStr = Arrays.toString(heapData);
        heapExp = "[38, 43, 39, 48, 47, 50, 46, 45, 44, 49, 42, 41, 40,";
        assertTrue(heapStr.contains(heapExp));
    ...
    public void stressTestsTwo() {
        ...
        while(heap.hasWork()) {
            ...
            int peekOne = heap.peek();
            int peekTwo =  (int) temp.peek();
            assertEquals(peekOne, peekTwo);


            int valOne = heap.next();
            int valTwo = (int) temp.remove();
